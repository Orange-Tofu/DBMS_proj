...
# These are basically page routing functions
@app.route('/')
def index():
...
def signup():
    if request.method == "POST":
        ...
        # To create user without Insert statement
...
def test():
    try:
        Test.query.all()
        return 'Successfully Intialised, Database is Connected.'
    except:
        return 'Error Occured, could not connect to database.'
